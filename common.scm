...
(define-module (common)
  ...
  #:export (var initial-var var/fresh state empty-state state-sub state->stream unify disunify
                typify distypify walk* reify reify/initial-var))
...
(define-record-type <state> (mk-state sub diseq types distypes) state?
  (sub state-sub)
  (diseq state-diseq)
  (types state-types)
  (distypes state-distypes))
...
;; Reification
(define-record-type <Ans> (mk-Ans term constraint) Ans?
  (term Ans-term)
